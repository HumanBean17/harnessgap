...
- Produces: `test/egress.test.ts` additionally asserts (a) any `import ... from 'node:child_process'` (or `require('child_process')`) in `src/` occurs **only** in files under `src/synthesizer/`; (b) no file in `src/` imports `node:http` / `node:https` / `undici` or calls global `fetch(`. The `src/egress.ts` header comment + the test's docstring state the gate guards the **default path only** — opt-in `synthesize` egress is bounded by `child_process` + the trusted agent CLI.
